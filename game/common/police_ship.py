...
class PoliceShip(Ship):
    ...
    def init(self, level=1, position=None):
        ...
        if level >= 3:
            ...
            self.weapon_damage = GameStats.get_ship_stat(ShipStat.weapon_damage, ModuleLevel.three)
            self.weapon_range = GameStats.get_ship_stat(ShipStat.weapon_range, ModuleLevel.three)
            self.sensor_range = GameStats.get_ship_stat(ShipStat.sensor_range, ModuleLevel.three)
